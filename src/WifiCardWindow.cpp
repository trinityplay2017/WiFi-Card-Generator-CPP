...
LRESULT CALLBACK WifiCardWindow::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        g_password = ::CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"password",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
            30, 120, 360, 34, hwnd, reinterpret_cast<HMENU>(IDC_PASSWORD),
            reinterpret_cast<LPCREATESTRUCT>(lParam)->hInstance, nullptr);

        HWND button = ::CreateWindowExW(0, L"BUTTON", L"Print WiFi Card",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            30, 185, 180, 40, hwnd, reinterpret_cast<HMENU>(IDC_PRINT),
            reinterpret_cast<LPCREATESTRUCT>(lParam)->hInstance, nullptr);
...
void WifiCardWindow::Paint(HWND hwnd, HDC hdc)
{
    ...
    const std::string payload = WifiQrGenerator::BuildPayload(ssid, password);

    RECT card{430, 25, client.right - 30, client.bottom - 30};
    DrawCard(hdc, card, ssid, password, payload);

    if (oldFont)
        ::SelectObject(hdc, oldFont);
}

void WifiCardWindow::DrawCard(HDC hdc, const RECT& rc, const std::string& ssid,
                              const std::string&, const std::string& payload)
{
    ...
    qr.right -= 25;
    qr.top += 55;
    qr.bottom = qr.top + std::min(qr.right - qr.left, rc.bottom - qr.top - 95);
    DrawQr(hdc, qr, payload);

    RECT name = rc;
    name.top = qr.bottom + 12;
    name.bottom = rc.bottom - 18;
...
void WifiCardWindow::DrawQr(HDC hdc, const RECT& rc, const std::string& payload)
{
    ...
    // Convert Win32 LONG dimensions to int explicitly.
    // This avoids std::min/std::max type ambiguity with MSVC.
    const int width =
    ...
    // Find the largest square that fits inside rc.
    const int available =
        (std::min)(width, height);

    // Calculate pixels per QR module.
    const int pixel =
        (std::max)(1, available / modules);

    // Actual QR bitmap size.
    const int qrSize =
        modules * pixel;

    // Center QR code inside the supplied rectangle.
    const int startX =
    ...
    // Draw complete white QR background including quiet zone.
    RECT qrBackground;
    ...
    for (int y = 0; y < size; ++y)
    {
        for (int x = 0; x < size; ++x)
        {
            ...
            moduleRect.left =
                startX + (x + quiet) * pixel;

            moduleRect.top =
                startY + (y + quiet) * pixel;

            moduleRect.right =
                moduleRect.left + pixel;

            moduleRect.bottom =
                moduleRect.top + pixel;
...
void WifiCardWindow::PrintCard(HWND hwnd)
{
    ...
    if (::StartDocW(pd.hDC, &di) > 0)
    {
        if (::StartPage(pd.hDC) > 0)
        {
            ...
            DrawCard(pd.hDC, page, ssid, password, payload);
            ::EndPage(pd.hDC);
        }
        ::EndDoc(pd.hDC);
    }
